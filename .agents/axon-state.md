...
| Next Steps | 1. Finalize Wave 6 by deciding whether observability counters should remain in-memory service counters or be exported via metrics backend/log sink integration, then add any missing assertions around emitted metadata fields 2. Capture manual real-draft-like verification evidence for create → attach → pick ingestion → reconnect → manual fallback → resume/end 3. Re-check expiry/reconnect verification gaps called out by `008b` and close any uncovered edge tests 4. After backend protocol stabilizes, execute `008c-extension-sync-adapters`; keep `010a-web-draft-kit-ui` limited to scaffold work until spec 010 is approved |
