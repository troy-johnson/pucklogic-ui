...
| Last Action | Continued `008b` implement-TDD with WebSocket pick-order guardrails: duplicate/out-of-turn pick numbers now emit explicit `error` events while valid next pick numbers emit `state_update`; router websocket tests cover the new sequencing behavior |
