...
| Last Action | Continued `008b` implement-TDD by moving websocket pick handling behind authoritative service/repository writes: added `accept_pick` service flow + repository session-progress persistence, plus websocket tests asserting persisted `sync_state` returns on `state_update` and duplicate/out-of-turn error propagation |
