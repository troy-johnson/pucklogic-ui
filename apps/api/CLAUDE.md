...
| Version | Trends API |
|---------|------------|
| **v1.0 (Phase 3)** | `GET /trends` returns pre-season Layer 1 scores (`breakout_score`, `regression_risk`, `confidence`, SHAP values) from `player_trends`. No paywall gate needed — all scores visible to free users. Feature spec: `docs/specs/007-feature-engineering-spec.md`. |
...
STRIPE_PRICE_ID=              # price_... draft pass SKU (Phase 2)
STRIPE_PRICE_KIT_PASS=        # price_... kit pass SKU (Milestone C)
