# apps/api/tests/scrapers/projection/test_lineup_experts.py
...
from scrapers.projection.lineup_experts import LineupExpertsScraper

FIXTURE_CSV = Path(__file__).parent.parent / "fixtures" / "lineup_experts_sample.csv"
...
class TestParseCsv:
    def test_returns_rows_with_player_name(self) -> None:
        rows = LineupExpertsScraper._parse_csv(FIXTURE_CSV.read_text())
        assert len(rows) >= 2
...
class TestIngest:
    ...
    def test_upserts_matched_players(self, mock_db: MagicMock) -> None:
        ...
        scraper = LineupExpertsScraper()
        with patch(
            "scrapers.projection.lineup_experts.fetch_players_and_aliases",
