...
async def compute_rankings(
    ...
    # 1. Auth — validate all access rights before touching the cache.
    # This prevents a warm-cache bypass where a second user replays a request
    # and receives data they are not authorized to see.

    # 1a. Batch-validate source_weights keys (single DB query)
    ...
    # 1c. Fetch scoring config (ownership-scoped)
    ...
    # 1d. Optionally validate league profile ownership
    ...
    # 2. Cache check (after auth passes)
    ...
    # 3. Fetch projections and run pipeline
