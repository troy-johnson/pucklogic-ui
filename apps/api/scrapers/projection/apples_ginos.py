# apps/api/scrapers/projection/apples_ginos.py
"""
Apples & Ginos projection parser (paste/upload mode).

Site: applesginos.com
Inspected: 2026-03-17

The applesginos.com domain does not resolve (DNS lookup fails) — the site
appears to be offline or not yet launched under this domain.  Even if the
domain resolves in future, the site's content is JavaScript-rendered and
would not be scrapeable via static HTTP fetches.

Users who have exported Apples & Ginos projection data can upload a CSV via
...
class ApplesGinosScraper(BaseProjectionScraper):
    """Parses user-supplied Apples & Ginos projection CSVs.
    ...
    SOURCE_NAME = "apples_ginos"
    DISPLAY_NAME = "Apples & Ginos"
    ...
    def _parse_csv(text: str) -> list[dict[str, Any]]:
        """Parse an Apples & Ginos projection CSV and return a list of row dicts.
    ...
    def ingest(self, csv_text: str, season: str, db: Any) -> int:
        ...
        for row in projection_rows:
            ...
            if player_id is None:
                ...
                logger.debug("ApplesGinos: unmatched player %r — skipping", player_name)
                continue
        ...
        logger.info(
            "ApplesGinos: upserted %d/%d projection rows for season %s",
            upserted,
            len(projection_rows),
            season,
        )
    ...
    async def scrape(self, season: str, db: Any) -> int:
        """Not implemented — Apples & Ginos cannot be auto-scraped.

        Raises:
            NotImplementedError: always.  Use ingest(csv_text, season, db) instead.
        """
        raise NotImplementedError(
            "Apples & Ginos does not publish scrapeable projections. "
